...
def plot_spectrogram(psd_data, frequencies, time_points, output_filename=None):
    ...
    plt.title('Spectrogram of PSD Data for 24 hours')

    # Set more frequent ticks for the x-axis (time in hours)
    time_tick_interval = 0.5  # Set tick interval in hours (0.5 = every 30 minutes)
...
# Generate the spectrogram for a full day from .zst files
def generate_spectrogram_from_zst_files(directory):
    psd_NS_list = []
    time_points = []
    frequencies = None
    minutes_per_file = 5
    total_minutes = 24 * 60  # Total minutes in 24 hours (1440 minutes)
    total_files = total_minutes // minutes_per_file  # Total number of 5-min files
    ...
    # Check if the number of files matches a full day of data
    if len(zst_files) < total_files:
        print(f"Warning: Not enough files for a full day. Found {len(zst_files)}, but expected {total_files}.")

    # Loop over each file and load the PSD data
    for i, zst_file in enumerate(zst_files[:total_files]):  # Limit to total_files (288 for 24 hours)
        ...
        # Store time point in hours (each file is 5 minutes)
    ...
    # Plot the spectrogram
    plot_spectrogram(psd_NS_matrix, frequencies, time_points, output_filename="full_day_spectrogram.png")


if __name__ == "__main__":
    input_directory = "../output/"  # Replace with the path to your directory containing .zst files
    generate_spectrogram_from_zst_files(input_directory)
