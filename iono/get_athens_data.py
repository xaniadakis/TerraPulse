...
char_names = ",".join([
    ...
    "zhalfNm", "yF2", "yF1", "yE", "scaleF2", "B0", "B1", "D1",
    "TEC",
...
def fetch_month(year, month, max_retries=3, delay=5):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # Create column names with _QD suffix where applicable
            col_names = []
            i = 0
            while i < len(raw_cols):
                name = raw_cols[i]
                if i + 1 < len(raw_cols):
                    next_token = raw_cols[i + 1]
                    if next_token in ["QD", "QF", "QE", "Q"]:  # these indicate a quality descriptor
                        col_names.append(name)
                        col_names.append(f"{name}_QD")
                        i += 2
                        continue
                col_names.append(name)
                i += 1
            ...
            for line in lines[data_start:]:
                ...
                if len(parts) != len(col_names):
                    raise ValueError(f"Mismatched value count: expected {len(col_names)}, got {len(parts)}")
            ...
            for col, pct in nan_percentages.items():
                ...
                if pct > threshold and debug:
                    tqdm.write(f"Tossing column '{col}' from '{desc}' because {pct * 100:.2f}% NaN")
